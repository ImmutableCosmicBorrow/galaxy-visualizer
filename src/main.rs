...
struct Explorer {
    id: ID,
    _pos: egui::Pos2,
    _exp_type: ExplorerType,
    bag: common_explorer::ExplorerBagContent,
}
...
impl eframe::App for GalaxyApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // Process delayed refreshes (100ms after sunray/asteroid)
        let mut refreshed = Vec::new();
        for (planet_id, sent_time) in &self.planets_to_refresh {
            // give the orchestrator a bit more time to process sunray/asteroid
            if sent_time.elapsed().as_millis() >= 300 {
                self.cmd_sender
                    .send(UiToOrchestratorCommand::GetPlanetSnapshot(*planet_id))
                    .ok();
                refreshed.push(*planet_id);
            }
        }
        self.planets_to_refresh
            .retain(|&(id, _)| !refreshed.contains(&id));
                ...
                match cmd {
                    ...
                    OrchestratorToUiUpdate::SupportedCombinations(explorer_id, combinations) => {
                        ...
                        if let Some(planet_id) = self.explorer_positions.get(&explorer_id).copied() {
                            ...
                            if Some(explorer_id) == self.pending_craft_explorer {
                    ...
                    OrchestratorToUiUpdate::SupportedResources(planet_id, resources) => {
                        ...
                        if let Some(expl_id) = self.pending_generate_explorer {
                            if let Some(current_planet) = self.explorer_positions.get(&expl_id).copied() {
                ...
                && let Some(pos) = self.pending_move_pos
            {
                ...
                if let Some(current_planet) = self.explorer_positions.get(&explorer_id).copied() {
                    ...
                    egui::Area::new(egui::Id::new("explorer_move_selector"))
                        ...
                        .show(ctx, |ui| {
                            ui.vertical(|ui| {
                                ui.label(format!("Move Explorer {explorer_id} from Planet {current_planet} to:"));
                                ui.separator();
                                ...
                                if neighbors.is_empty() {
                                    ...
                                    for nid in &neighbors {
                                        if ui.button(format!("Planet {}", nid)).clicked() {
                                            ...
                                            println!("→ Sending ManualMoveExplorer(explorer={}, from={}, to={})", explorer_id, current_planet, nid);
                                            let _ = self.cmd_sender.send(UiToOrchestratorCommand::ManualMoveExplorer(explorer_id, current_planet, *nid));
                        ...
                        ui.vertical(|ui| {
                            ...
                            if self.resource_options.is_empty() {
                                ...
                                if let Some(res) = chosen_resource {
                                    println!("→ Sending ManualExplorerCraftsRes(explorer={}, resource={:?})", expl_id, res);
                                    let _ = self.cmd_sender.send(UiToOrchestratorCommand::ManualExplorerCraftsRes(expl_id, res));
                                    let _ = self.cmd_sender.send(UiToOrchestratorCommand::GetExplorerSnapshot(expl_id));
                                    let _ = self.cmd_sender.send(UiToOrchestratorCommand::GetExplorersPosition);
                                    self.pending_generate_explorer = None;
                                    self.resource_options.clear();
                                }
                ...
                if self.combination_options.is_empty() {
                    if let Some(current_planet) = self.explorer_positions.get(&expl_id).copied() {
                        if let Some(cached) = self.combination_cache.get(&current_planet) {
                            self.combination_options = cached.clone();
                        } else {
                            // request supported combinations for this explorer (will populate cache)
                            let _ = self.cmd_sender.send(UiToOrchestratorCommand::SupportedCombinations(expl_id));
                        }
                    }
                }
                        ...
                        ui.vertical(|ui| {
                            ...
                            if self.combination_options.is_empty() {
                                ...
                                if let Some(combo) = chosen_combo {
                                    println!("→ Sending ManualExplorerCombineRes(explorer={}, combo={:?})", expl_id, combo);
                                    let _ = self.cmd_sender.send(UiToOrchestratorCommand::ManualExplorerCombineRes(expl_id, combo));
                                    let _ = self.cmd_sender.send(UiToOrchestratorCommand::GetExplorerSnapshot(expl_id));
                                    let _ = self.cmd_sender.send(UiToOrchestratorCommand::GetExplorersPosition);
                                    self.pending_craft_explorer = None;
                                    self.combination_options.clear();
